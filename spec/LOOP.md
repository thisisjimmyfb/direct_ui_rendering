...
- Add unit test `TessellateStringTest.NonPrintable_In_MixedString_PositionsAdvanceByGlyphCell`: verify that in a string like "A\tB" each character (including non-printable ones) advances the cursor by `GLYPH_CELL`, so non-printable clamping to space UVs does not silently skip position advancement or double-advance.
