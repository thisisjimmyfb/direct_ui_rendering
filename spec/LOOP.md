...
- Add unit test for `lightViewProj` orthographic invariant — In `test_math.cpp`, extend `LightFrustumTest` with a case that projects all 8 room corners through `scene.lightViewProj()` and asserts each `clip.w ≈ 1.0` (to 1e-4). An orthographic projection leaves W unchanged; if the matrix accidentally becomes perspective, W varies and the shadow-map depth comparisons break silently.
- Add unit test for `lightViewProj` Z-spread tightness — In `test_math.cpp`, extend `LightFrustumTest` with a case that projects all 8 room corners through `scene.lightViewProj()`, computes min and max NDC Z, and asserts `(maxNdcZ - minNdcZ) > 0.5`. This ensures the near/far planes are tight enough that the full depth range is exploited; a loose frustum (e.g. nearZ≈0, farZ=100) would pack all geometry into a tiny slice of the depth buffer, reducing shadow precision.
