...
- Add unit test: `scene.light().color` all-positive — In `test_math.cpp` under `LightFrustumTest`, assert that all three RGB components of `scene.light().color` are greater than 0. Zeroing any channel silently extinguishes the corresponding wavelength from all diffuse and specular contributions, producing incorrect rendering with no compile-time or runtime error.
- Add unit test: `scene.light().direction` not parallel to lookAt up vector — In `test_math.cpp` under `LightFrustumTest`, assert `|dot(scene.light().direction, vec3(0,1,0))| < 1.0 - 1e-4`. A direction of exactly `(0,±1,0)` would make the shadow-map `glm::lookAt` degenerate (up vector collinear with the view direction), producing a NaN matrix and invisible shadows everywhere.
