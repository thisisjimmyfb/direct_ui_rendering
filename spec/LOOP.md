...
- Build the app via /scripts/build.sh --platform=windows and /scripts/build.sh --platform=android to ensure project builds properly for windows and android.
- Run tests via /scripts/test.sh --platform=windows and /scripts/test.sh --platform=android. If there are any test failures, please investigate and fix the failure if the fix is small. If the fix will be big, please identify tasks to address the problem, and then append the task to the ['Pending Tasks'](#Pending-Tasks). 
- Check Vulkan errors by running /build/Debug/direct_ui_rendering.exe --timeout 10 and then read the output and fix all Vulkan Validation Layer errors. 
