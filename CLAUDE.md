...
│   ├── app.cpp                       # App implementation: window, event loop, render loop (522 lines)
...
├── shaders/
│   ├── common.glsl                   # Shared GLSL functions: sampleShadowPCF (with depth bias), hash, noisePerlin
│   ├── ubo_structs.glsl              # Shared UBO definitions: SceneUBO, SurfaceUBO (included by all shaders that use uniform buffers)
...
│   ├── test_app_helper.h             # Shared AppTestHelper for accessing App private members in tests
│   ├── test_pixel_helpers.h          # Shared pixel unpacking and sampling utilities for render tests
